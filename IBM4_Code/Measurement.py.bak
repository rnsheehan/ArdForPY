        ...
        while True:
            if supervisor.runtime.serial_bytes_available:   # Listens for a serial command
                ...
                if command.startswith("o"):                 # If the command starts with 'o' it knows it is an output (Write)
                    ...
                    # Previously this command was "i", but i changed it to be more consistent with the Arduino Micro
...
def hardware_SPI_test(clock_pin, data_pin):
    # check to see if a certain pin-pair combination can be used for hardware SPI
    # R. Sheehan 9 - 11 - 2020

    FUNC_NAME = ".hardware_SPI_test()" # use this in exception handling messages
    ERR_STATEMENT = "Error: " + MOD_NAME_STR + FUNC_NAME

    try:
        print("Clock Pin: ",clock_pin)
        print("Data Pin: ",data_pin)
        if is_hardware_spi(clock_pin, data_pin):
            print("This pin combination is hardware SPI")
        else:
            print("This pin combination is not hardware SPI")
    except Exception as e:
        print(ERR_STATEMENT)
        print(e)
...
def is_hardware_spi(clock_pin, data_pin):
    ...
    FUNC_NAME = ".is_hardware_spi()" # use this in exception handling messages
    ERR_STATEMENT = "Error: " + MOD_NAME_STR + FUNC_NAME
...
def T_Meas_Bridge():
    # Measure temperature using a thermistor inside a Wheatstone Bridge
    # For the bridge that I built
    # R_{1} = 9.87 kOhm, R_{2} = 9.89 kOhm
    # R_{3} = 9.86 kOhm, R_{x} = 10 kOhm at T = 25 C
    # Supply voltage will be V_{s} = 3.3 V
    # IBM4 connections
    # A2 -> V_{s}, A3 -> V_{g1}, A4 -> V_{g2}
    # V_{b1} ~ V_{s} / 2 since it's a fixed voltage divider
    # R. Sheehan 10 - 11 - 2020

    FUNC_NAME = "T_Meas_Bridge.()" # use this in exception handling messages
    ERR_STATEMENT = "Error: " + MOD_NAME_STR + FUNC_NAME

    try:
        Vgnd = get_voltage(Vin1) # supply voltage
        Vs = get_voltage(Vin2) # supply voltage
        Vb1 = get_voltage(Vin3) # voltage out of fixed voltage divider
        Vb2 = get_voltage(Vin4) # voltage out of fixed voltage divider
        Vb = abs(Vb1 - Vb2) # voltage across Wheatstone bridge
        R = 10 # Wheatstone Bridge Resistances in kOhm
        ratio = Vb / Vs # ratio of input voltage to Wheatstone Bridge voltage
        numer = 0.5-ratio
        denom = 0.5+ratio
        Rx = ( numer / denom ) * R # resistance across thermistor
        print("Wheatstone Bridge Measurement")
        print("Ground: ", Vgnd)
        print("Supply Voltage: ", Vs)
        print("Vb1: ", Vb1, ", 2 Vb1: ", 2.0*Vb1)
        print("Vb2 ", Vb2)
        print("Bridge Voltage Vb: ",Vb)
        print("Thermistor Resistance: ",Rx,"\n")
    except ValueError as e:
        print(ERR_STATEMENT)
        print(e)

def T_Meas_Divider():
    # Measure temperature using a thermistor inside a voltage divider
    # For the voltage divider   
    # R_{1} = 9.86 kOhm, R_{x} = 10 kOhm at T = 25 C
    # Supply voltage will be V_{s} = 3.3 V
    # R. Sheehan 10 - 11 - 2020
    
    FUNC_NAME = "T_Meas_Divider.()" # use this in exception handling messages
    ERR_STATEMENT = "Error: " + MOD_NAME_STR + FUNC_NAME
    ...
    try:
        ...
        Vb2 = get_voltage(Vin4) # voltage out of fixed voltage divider
        R = 10 # Wheatstone Bridge Resistances in kOhm
        Rx = ( Vb2 / (Vs - Vb2) ) * R # resistance across thermistor
        print("Voltage Divider Measurement")
        print("Ground: ", Vgnd)
        print("Supply Voltage: ", Vs)
        print("Divider Voltage Vb: ",Vb2)
        print("Thermistor Resistance: ",Rx,"\n")
    except ValueError as e:
        print(ERR_STATEMENT)
        print(e)
